...
function entityCategory(value?: string): SpeakerCategory | undefined {
  ...
  if (["observer", "obs"].includes(code)) return "Observer";
  if (["un", "unentity", "unagency"].includes(code)) return "UN Entity";
  if (["igo", "intergovernmentalorganization"].includes(code)) return "Intergovernmental Organization";
